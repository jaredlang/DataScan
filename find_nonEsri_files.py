...
OUTPUT_HEADERS = ["Source Type", "Data Source", "SDE Name", "SDE Conn"]
...
def get_file_ext(dataSource):
    parts = dataSource.split('\\')
    for p in reversed(parts):
        fname,fext = os.path.splitext(p)
        if fext is not None and len(fext) > 0:
            return fext
    return None


def find_files_by_ext(xlsFolder, fileExts, xlsOutput):
    dataFileSet = set()
    ...
    print('**** The compiled data file list: %s' % xlsOutput)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Find all non-ESRI data files')
    ...
    parser.add_argument('-e','--ext', help='File Extensions', required=True)
    ...
    fileExts = params.ext.split(',')

    find_files_by_ext(params.xls, fileExts, params.dsf)
